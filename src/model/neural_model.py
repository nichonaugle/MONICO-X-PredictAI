...
limited_features = ['0718.Cylinder_04_Transformer_Secondary_Output', 
...
def init() -> None:
    global model
    model_dir = os.getcwd() + "/src/model/trained/RFModelV1.pkl"
...
def get_indexes():
    ...
    for i in range(len(features)):
        if features[i] in limited_features:
            index_array.append(i)
    
    return index_array

def run_model(data) -> str:
    index_array = get_indexes()
    indexed_data = [data[i] for i in index_array]

    if len(limited_features) != len(indexed_data):
        raise ValueError("ERROR: Inference cannot be made! Indexed data input does not match expected input size.")

    # Convert to NumPy array and reshape if necessary
    indexed_data = np.array(indexed_data).reshape(1, -1)
